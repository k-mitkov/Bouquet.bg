...
namespace Bouquet.Services.Bouquet
{
    public class FlowerService : IFlowerService
    {
        ...
        public FlowerService(BouquetContext dbContext, IMapper mapper)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<Response> GetFlowersAsync()
        {
            var flowers = await _dbContext.Flowers.ToListAsync();

            if (flowers == null || !flowers.Any())
            {
                return new Response { Status = StatusEnum.Failure, Message = "Flowers not found" };
            }

            return new Response<IEnumerable<FlowerDTO>> { Status = StatusEnum.Success, Data = _mapper.Map<IEnumerable<FlowerDTO>>(flowers) };
        }
